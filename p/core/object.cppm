...
{
public:
    HeapObject(ObjectTypeSymbol* type_, int32_t fieldCount_, int32_t size_);
    Object* Clone() const override;
    std::string KindStr() const override { return "heap_object"; }
    ObjectTypeSymbol* GetType() const { return type; }
    void SetType(ObjectTypeSymbol* type_) { type = type_; }
...
{
public:
    ArrayObject(ArrayTypeSymbol* arrayType_, int32_t length_, int32_t size_);
    Object* Clone() const override;
    ...
    ArrayTypeSymbol* GetArrayType() const { return arrayType; }
    void SetArrayType(ArrayTypeSymbol* arrayType_) { arrayType = arrayType_; }
    int32_t Size() const { return size; }
    void SetSize(int32_t size_) { size = size_; }
private:
