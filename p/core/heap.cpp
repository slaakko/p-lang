    ...
    ExecutionContext* context)
{
    ...
    else
    {
        ...
        parent->SetElement(index, newObject, context);
        ObjectTypeSymbol* objectType = newObject->GetType();
        int32_t fieldCount = objectType->Fields().size();
...
void Heap::DisposeObjects(ExecutionContext* context)
{
    ...
    {
        ObjectTypeSymbol* objectType = disposeObject->GetType();
        std::string disposeMethodName = objectType->Name() + ".Dispose";
        ExternalSubroutineSymbol* disposeMethod = GetExternalSubroutineNoThrow(disposeMethodName);
        if (disposeMethod)
        {
...
HeapObject* Heap::Allocate(ObjectTypeSymbol* objectType, ExecutionContext* context)
{
    int32_t fieldCount = objectType->Fields().size();
...
ArrayObject* Heap::AllocateArray(ArrayTypeSymbol* arrayType, int32_t arrayLength, ExecutionContext* context)
{
